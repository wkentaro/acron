...
1. **Overlap prevention** — agent runs take minutes and may exceed the interval; acron enforces single-instance so two runs do not fight over the same repo (systemd will not restart an already-active unit, but launchd has nothing comparable, so acron enforces it uniformly on both).
...
3. **Log capture + run history** — an agent run is a narrative ("what did it actually do?"), not a pass/fail; acron captures output and keeps one history, surfaced via `acron logs` (systemd has journald and launchd writes to log files, but neither is a uniform queryable cross-OS run history).
