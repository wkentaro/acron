...
A command-line runtime for unattended agent runs that happens to be scheduled cross-OS. acron translates a Job into the native OS scheduler (systemd timer on Linux, launchd on macOS) and stays in the runtime path: the generated unit runs `acron run <job>`, not the agent directly, so acron supervises every firing in-process. Its reason to exist is not cross-OS scheduling (that is just the delivery mechanism) but giving a long-running agent one uniform set of guarantees: overlap prevention, timeout, and log capture with run history. The OS schedulers cover these unevenly (systemd natively via `RuntimeMaxSec`, `ExecCondition`, and journald; launchd not at all), so acron owns them in-process to get identical supervision semantics and one queryable history (`history.jsonl`, read by `acron status`/`history`/`logs`) on both. Around that sits an ergonomics layer (one declarative config, easy env/PATH/keys, working directory as a field) that makes scheduling a single agent run painless.
...
**Run**:
A single execution of an agent triggered by the scheduler at a fired time. A Run ends with a status: `success`, `failure`, `timeout`, `skipped` (the firing was dropped without running the agent — either the previous Run still held the lock, or the Condition was not met), or `interrupted` (the operator aborted it with Ctrl-C, SIGINT/SIGTERM, before it reached a verdict). While a Run is in flight it has no recorded status; live CLI surfaces derive its current phase from the held lock, showing `condition` while the Condition is still being checked and `running` once the agent has started. A Run is identified by its fired time, the stable handle the CLI uses to select it (e.g. `acron logs`), which resolves the in-flight Run too once its log exists.
