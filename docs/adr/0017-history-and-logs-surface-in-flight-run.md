...
`acron history` shows a Job's currently-running Run as a `running` row at the top
of the timeline, and `acron logs <job> <timestamp>` resolves that Run by its
fired time. Both read the same live state `status` already uses (ADR 0014): the
held lock, with the start time recovered from the in-flight log name. Nothing new
is written — a Run still gains its `history.jsonl` record only at completion (ADR
0007), and `running` is never a stored status (CONTEXT: Run).
...
- **`history` adds at most one `running` row per Job**, since one lock means one
  in-flight Run. It sorts to the top by start time and is subject to `--limit`
  like any other row (it is the newest, so the cap never drops it). `STATUS` is
  `running`; `DURATION` is `—` (no final duration yet, as for a skip, ADR 0016);
  `PASSED` is elapsed-since-start, which doubles as how long it has been running.
- **A Run still in its Condition check is omitted**, not shown. Its start time is
  ...
  `status` legitimately shows it because that is a one-row-per-Job current-state
  view, not a time-keyed log.
...
- `runHistory` probes `RunningSince` per Job (the same cheap lock read `status`
  does) and appends one synthesized row when the start is known; the running row
  renders `running`/`—` instead of going through `renderStatus`/`renderRunDuration`,
  since `running` is not a stored `Status`.
